...
import { updateOpenManufacturing } from "../../action";

export default function EditOpenManufacturing() {
  const params = useParams();
  const openManufacturingId = params.id;
  ...
  const [
    updateOpenManufacturingState,
    updateOpenManufacturingAction,
    isPendingUpdateOpenManufacturing,
  ] = useActionState(updateOpenManufacturing, INITIAL_STATE_OPEN_MANUFACTURING);
      ...
      queryFn: async () => {
        const result = await supabase
          ...
          .select(
            ...
            product_units_id,product_units (
  ...
  useEffect(() => {
    ...
    form.setValue(
      ...
      String(
        (
          openManufacturingData?.product_units as {
    ...
    form.setValue(
      "product_units_id",
      String(openManufacturingData?.product_units_id ?? ""),
    );
  ...
  useEffect(() => {
    if (updateOpenManufacturingState?.status === "error") {
      toast.error("Update Open Manufacturing Failed", {
        description: updateOpenManufacturingState.errors?._form?.[0],
      });
    }

    if (updateOpenManufacturingState?.status === "success") {
      toast.success("Update Open Manufacturing Success");
      queryClient.refetchQueries({ queryKey: ["open_manufacturing"] });
      router.push("/production/open-manufacturing");
    }
  }, [updateOpenManufacturingState, queryClient, router]);
  ...
  return (
    <CardOpenManufacturing
      ...
      onSubmit={onSubmit}
      isPending={isPendingUpdateOpenManufacturing}
